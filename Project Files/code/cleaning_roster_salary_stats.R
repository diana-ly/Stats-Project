...
# The purpose for this join is for player_stats to have the salary column for 
# each player. The subset is for to remove the repeated columns. 
...
# for the 2014-2015 season. Thus we had to obmit the players without salary 
# from the dataframe, since they do not contribute to the analysis. 
...
# position data frame is to add the position to stats_salary. We just subset 
# the Player and the Pos column from roster. 
...
# Since some players get traded several time during the season, for example, 
# A.J. Price was one the Cavaliers, Pacers, and Suns, we decided to only use 
# one stat for each player. We used the logical vector to subset the data 
# frame. First we made two empty vector, character vector and logical vector. 
# The for loop goes through the Players in stats_salary_pos and append TRUE if 
# the player is not in the unique_players vecotr and append the player 
# character string into unique_players; append FALSE if the player character 
# string is already in unique_players. 
